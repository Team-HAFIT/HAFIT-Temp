...
def check_knee_forward(world_landmarks):        #무릎이 앞으로 갔는지 확인
    ...
    left_knee_forward = left_knee[0] > left_ankle[0]  # 수정된 코드
    right_knee_forward = right_knee[0] > right_ankle[0]  # 수정된 코드
...
def graph_update_thread(landmark_queue):                #rm
    app = QtWidgets.QApplication([])
    w = gl.GLViewWidget()
    w.show()
    w.setWindowTitle('3D Pose Visualization')
    w.setCameraPosition(distance=2, elevation=20, azimuth=-90)  # Update camera position and angles

    g = gl.GLGridItem()
    w.addItem(g)

    scatter = gl.GLScatterPlotItem(pos=np.zeros((33, 3), dtype=np.float32), color=pg.glColor('r'), size=12)
    w.addItem(scatter)

    # Add lines for pose connections
    lines = []
    for connection in mp.solutions.pose.POSE_CONNECTIONS:
        line = gl.GLLinePlotItem(width=3)
        w.addItem(line)
        lines.append(line)
    ...
    while True:
        try:
            ...
            # camera_azimuth += 1  # Change azimuth by 1 degree per frame
            # w.setCameraPosition(distance=5, elevation=0, azimuth=camera_azimuth)
